...
class Solver_ES:
    ...
    def SCF(self, L=None, ts=None, ls=None, rn=None, ln=None, r0n=None, l0n=None, diis=None, force_alpha=True):
        """
        ...
        # initialize dictionary with amplitudes
        ...
        X2 = np.zeros((nbr_states, nbr_states))
        Ep = np.zeros((nbr_states, 2))
        ...
        while Dconv > self.conv_thres:
            ...
            for n in range(1, nbr_states):
                ...
                # Store right and left excited states correlation energies Ep^r and Ep^l
                Ep[n, 0] = En_r + self.EHF
                Ep[n, 1] = En_l + self.EHF
            ...
            if diis == 'all':
                ...
                vec = np.concatenate((np.ravel(ts), np.ravel(ls), [np.ravel(r) for r in rnew][0],
                                      [np.ravel(l) for l in lnew][0], r0new[0], l0new[0]))
                vec = amp_diis.update(vec)
                vec_0 = vec[-2 * len(r0new):]  # r0 and l0 amplitudes
                vec = np.split(vec[:-2 * len(r0new)], 2*(len(lnew)+1))  # other amp
                # extract all new amplitudes
                ts = vec[0].reshape((nocc, nvir))
                ls = vec[1].reshape((nocc, nvir))
                for i in range(len(lnew)):
                    rnew[i] = vec[2+i].reshape((nocc, nvir))
                    lnew[i] = vec[2+i+len(rnew)].reshape((nocc, nvir))
                    r0new[i] = vec_0[:len(r0new)]
                    l0new[i] = vec_0[len(r0new):]
                del vec
